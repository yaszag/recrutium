...
export class PaymentsComponent implements OnInit, OnDestroy {
  ...
  public lstPayment!: Array<payment>;
  ...
  public sortData(sort: Sort) {
    const data = this.lstPayment.slice();

    if (!sort.active || sort.direction === '') {
      this.lstPayment = data;
    } else {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      this.lstPayment= data.sort((a, b) => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    this.data.lstFiles().subscribe((data: apiResultFormat) => {
      ...
      data.data.map((res: payment, index: number) => {
        ...
        if (index >= this.skip && serialNumber <= this.limit) {
          ...
          this.lstPayment.push(res);
          this.serialNumberArray.push(serialNumber);
        }
      });
      this.dataSource = new MatTableDataSource<payment>(this.lstPayment);
